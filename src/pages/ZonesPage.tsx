    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowAddModal(true)}>
          <Plus className="mr-2 h-4 w-4" /> Add {activeTab === 'zones' ? 'Zone' : 'Store'}
        </Button>
      </div>
            ...
            <Card key={zone.id} className={selectedZoneId === zone.id ? 'ring-2 ring-primary-500' : ''} onClick={() => setSelectedZoneId(zone.id)}>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => deleteZone(zone.id)} className="p-1 hover:bg-gray-100 rounded text-red-600"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
              <div>
