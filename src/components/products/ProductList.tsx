    ...
    <Card key={product.id} className="p-4 mb-3">
        ...
        <div className="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-1">
              <h3 className="font-semibold text-base truncate">{product.nameEn}</h3>
      ...
      <div className="md:hidden mt-3 pt-3 border-t">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-semibold">{formatCurrency(product.variants[0]?.price || 0)}</p>
            {product.variants[0]?.discount && (
              <p className="text-xs text-green-600">{product.variants[0].discount}% off</p>
            )}
          </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {showAddButton && onAddProduct && (
          <Button onClick={onAddProduct} className="w-full sm:w-auto">
            ...
            Add {activeTab === 'hub' ? 'Hub Product' : 'Store Product'}
          </Button>
        )}
      </div>
            ...
            return (
              <Card key={tab.id} className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`p-3 rounded-lg ${tab.id === 'hub' ? 'bg-blue-100' : 'bg-green-100'}`}>
                    <Icon className={`h-6 w-6 ${tab.id === 'hub' ? 'text-blue-600' : 'text-green-600'}`} />
                  </div>
                </div>
              </Card>
            );
