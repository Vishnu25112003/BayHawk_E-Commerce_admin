...
import { CustomerSearchCard, ProductSelector } from './';
...
const preOrderSchema = z.object({
  ...
  preOrderType: z.enum(['advance_booking', 'rare_product', 'bulk_order', 'subscription']),
...
export function PreOrderForm({ moduleType, products, hubs, onSubmit }: PreOrderFormProps) {
  ...
  const [selectedPreOrderType, setSelectedPreOrderType] = useState<string>('');
  ...
  const updateDateLimits = (type: string) => {
  ...
  const total = subtotal - eliteDiscount + deliveryCharges;
  ...
  const handlePreOrderTypeChange = (type: string) => {
    setSelectedPreOrderType(type);
    setValue('preOrderType', type as any);
  ...
  const handleFormSubmit = async (data: PreOrderFormData) => {
    setIsLoading(true);
    try {
      await onSubmit({ ...data, total, remainingAmount, isEliteMember } as any);
      reset();
  ...
  // Filter products for rare product orders
  const availableProducts = selectedPreOrderType === 'rare_product' 
    ? products.filter(p => p.isRare) 
    : products;
                  ...
                  <div key={field.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {item?.isRareProduct && (
                          <Badge variant="warning">Rare Product</Badge>
                        )}
                      </div>
              ...
              {isEliteMember && (
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div className="flex items-center gap-2 text-yellow-800 mb-2">
                    <Crown className="h-5 w-5" />
                    <span className="font-semibold">Elite Member Pre-Order Benefits</span>
                  </div>
                  <div className="text-sm text-yellow-700">
                    <p>Total Savings: {formatCurrency(eliteDiscount + (50 - deliveryCharges))}</p>
                    <p className="text-xs mt-1">Includes product discount and free delivery</p>
                  </div>
                </div>
              )}
            </div>
