...
export function OrderDetails({ order, className = '', isEditing = false, onPaymentRecord, onRefundRecord, onAssignThirdParty }: OrderDetailsProps) {
  ...
  const handleRecordRefund = async (refundData: any) => {
    if (onRefundRecord) {
            ...
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm text-red-800">
                  <span className="font-medium">Refunds Processed:</span> {formatCurrency(refundedAmount)} has been refunded to customer
                </p>
              </div>
            </div>
          )}
